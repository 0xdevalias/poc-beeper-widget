...
export default function Name() {
  ...
  async function fetchData() {
    const response: StateEvent<any>[] =
      await widgetApi.receiveStateEvents("m.room.name");
  ...
  return (
    <>
      <Back />
      {nameResponse.length && (
        <p className="mt-4">Room name: {nameResponse[0].content.name}</p>
      )}
      ...
      <form onSubmit={setName}>
        ...
        />
        <button className="block mt-2 bg-black text-white p-2 border border-black hover:bg-white hover:text-black">
          Set name
        </button>
      </form>
