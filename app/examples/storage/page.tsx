...
export default function Storage() {
  ...
  const [storedText, setStoredText] = useState(
    localStorage.getItem("widget_example_text") || "",
  );
  ...
  return (
    <>
      ...
      <p className="mt-4">
        {
          "Using localStorage, a widget can access data across any of the user's chats."
        }
      </p>
      <p className="mt-4">
        This is useful if you want the user to add their own information, such
        as preferences and API keys.
      </p>

      {storedText && <p className="mt-4">Current value: {storedText}</p>}

      <form onSubmit={setLocalStorage}>
        <input
          className="block border border-black p-2 w-3/4 text-black"
          value={text}
