...
export default function Redaction() {
  ...
  async function sendMessage(event: any) {
    ...
    const sendMessageResponse = await widgetApi.sendRoomEvent(
      "m.room.message",
      {
        msgtype: "m.text",
  ...
  return (
    <>
      ...
      <button
        onClick={sendMessage}
        className="mt-2 bg-white text-black p-2 border border-black hover:bg-gray-100 block"
      >
        Send a test message
      </button>
      <button
        className="mt-2 bg-black text-white p-2 border border-black hover:bg-white hover:text-black block disabled:opacity-50 disabled:cursor-not-allowed"
